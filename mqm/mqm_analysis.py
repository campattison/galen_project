...
def load_mqm_data(filepath):
    ...
    # Clean column names - updated for new format with Quality Rating columns
    df.columns = ['blank', 'Text', 'Chunk', 'Translation', 'Model', 'Word Count', 'APT', 'TQS',
                  'Quality_Rating', 'Critical_Error', 'Final_Quality_Rating',
    ...
    # Convert Critical_Error to boolean (handles "TRUE"/"FALSE" strings)
    df['Critical_Error'] = df['Critical_Error'].astype(str).str.upper() == 'TRUE'
    return df
...
def create_tqs_heatmap(data, filename):
    ...
    # Compact figure - leave room on right for colorbar
    fig, axes = plt.subplots(1, 2, figsize=(7.5, 6))
    plt.subplots_adjust(left=0.08, right=0.82, wspace=0.08)

    text_labels = {'Comp': 'Comp.', 'Mixtures': 'Mix.'}
    ...
    for ax_idx, text in enumerate(['Comp', 'Mixtures']):
        ...
        # Create heatmap with smaller aspect ratio for compact cells
        im = ax.imshow(pivot.values, cmap='RdYlGn', vmin=0, vmax=100, aspect=0.4)
        ...
        ax.set_xticklabels([MODEL_NAMES[m] for m in pivot.columns], fontsize=9, fontweight='bold')
        ax.set_yticks(range(len(pivot)))

        # Only show chunk labels on left panel
        if ax_idx == 0:
            ax.set_yticklabels([f'{c}' for c in pivot.index], fontsize=9)
            ax.set_ylabel('Chunk', fontsize=10, fontweight='bold')
        else:
            ax.set_yticklabels([])

        # Italicized abbreviated title
        ax.set_title(text_labels[text], fontsize=11, fontweight='bold', fontstyle='italic', pad=8)
    ...
    # Add colorbar with proper spacing (outside the plot area)
    cbar_ax = fig.add_axes([0.85, 0.15, 0.025, 0.7])
    cbar = fig.colorbar(im, cax=cbar_ax)
    cbar.set_label('TQS', fontweight='bold', fontsize=9)
    cbar.set_ticks([0, 50, 100])
    cbar.set_ticklabels(['0', '50', '100'])
    cbar.ax.tick_params(labelsize=8)

    # No main title (will be in caption)

    plt.savefig(filename, dpi=300, facecolor='white')
    plt.close()

    # Reset font to default for other charts
...
# ============================================================================
# CHART 5: Quality Rating Distribution (Pass/Fail)
# ============================================================================

def create_quality_rating_chart(data, filename):
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))

    # Chart 1: Final Quality Rating distribution
    ax1 = axes[0]
    models = ['claude', 'gemini', 'openai']
    rating_order = ['HIGH PASS', 'LOW PASS', 'FAIL']
    rating_colors = {'HIGH PASS': '#2d6a4f', 'LOW PASS': '#f9c74f', 'FAIL': '#d62828'}
    ...
    for i, rating in enumerate(rating_order):
        ...
        for model in models:
            ...
            count = (model_df['Final_Quality_Rating'] == rating).sum()
            counts.append(count)
        bars = ax1.bar(x + (i - 1) * width, counts, width, label=rating,
                      color=rating_colors[rating], edgecolor='white', linewidth=1)
        # Add value labels
        for bar, val in zip(bars, counts):
            if val > 0:
                ax1.annotate(f'{int(val)}', xy=(bar.get_x() + bar.get_width()/2, bar.get_height()),
                            ha='center', va='bottom', fontsize=10, fontweight='bold')

    ax1.set_xticks(x)
    ax1.set_xticklabels([MODEL_NAMES[m] for m in models], fontsize=12, fontweight='bold')
    ax1.set_ylabel('Number of Chunks', fontsize=12, fontweight='bold')
    ax1.set_title('Final Quality Rating Distribution', fontsize=14, fontweight='bold', pad=10)
    ax1.legend(loc='upper right', framealpha=0.9)
    ax1.set_ylim(0, max([data[data['Model'] == m]['Final_Quality_Rating'].value_counts().max() for m in models]) + 3)

    # Chart 2: Pass Rate (combined HIGH PASS + LOW PASS that aren't FAIL)
    ax2 = axes[1]
    ...
    for model in models:
        model_df = data[data['Model'] == model]
        total = len(model_df)
        passes = ((model_df['Final_Quality_Rating'] == 'HIGH PASS') |
                  (model_df['Final_Quality_Rating'] == 'LOW PASS')).sum()
        pass_rates.append(passes / total * 100)
        fail_rates.append((total - passes) / total * 100)

    bars_pass = ax2.bar(x, pass_rates, 0.6, label='Pass (High + Low)', color='#2d6a4f',
                        edgecolor='white', linewidth=1)
    bars_fail = ax2.bar(x, fail_rates, 0.6, bottom=pass_rates, label='Fail',
                        color='#d62828', edgecolor='white', linewidth=1)

    # Add percentage labels
    for i, (pr, fr) in enumerate(zip(pass_rates, fail_rates)):
        ax2.annotate(f'{pr:.0f}%', xy=(i, pr/2), ha='center', va='center',
                    fontsize=12, fontweight='bold', color='white')
        if fr > 5:  # Only show if there's room
            ax2.annotate(f'{fr:.0f}%', xy=(i, pr + fr/2), ha='center', va='center',
                        fontsize=12, fontweight='bold', color='white')

    ax2.set_xticks(x)
    ax2.set_xticklabels([MODEL_NAMES[m] for m in models], fontsize=12, fontweight='bold')
    ax2.set_ylabel('Percentage of Chunks', fontsize=12, fontweight='bold')
    ax2.set_title('Pass Rate by Model', fontsize=14, fontweight='bold', pad=10)
    ax2.legend(loc='upper right', framealpha=0.9)
    ax2.set_ylim(0, 105)

    fig.suptitle('Quality Rating Analysis', fontsize=15, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(filename, bbox_inches='tight', dpi=300, facecolor='white')
    plt.close()
    print(f"Saved: {filename}")

create_quality_rating_chart(df, 'charts/chart5_quality_ratings.png')

# ============================================================================
# QUALITY RATING ANALYSIS (Console Output)
# ============================================================================

print("\n" + "-" * 60)
print("QUALITY RATING ANALYSIS")
print("-" * 60)

print("\nFinal Quality Ratings by Model:")
for model in ['claude', 'gemini', 'openai']:
    model_df = df[df['Model'] == model]
    print(f"\n{MODEL_NAMES[model]}:")
    for rating in ['HIGH PASS', 'LOW PASS', 'FAIL']:
        count = (model_df['Final_Quality_Rating'] == rating).sum()
        pct = count / len(model_df) * 100
        print(f"  {rating:12}: {count:2} ({pct:5.1f}%)")

print("\n\nCritical Errors by Model:")
for model in ['claude', 'gemini', 'openai']:
    model_df = df[df['Model'] == model]
    critical_errors = (model_df['Critical_Error'] == True).sum()
    pct = critical_errors / len(model_df) * 100
    print(f"  {MODEL_NAMES[model]:10}: {critical_errors:2} chunks with critical errors ({pct:.1f}%)")
...
report += """
═══════════════════════════════════════════════════════════════════════════════
                          6. QUALITY RATING ANALYSIS
═══════════════════════════════════════════════════════════════════════════════

    Final Quality Rating Distribution:

    ┌─────────────┬────────────┬────────────┬──────────┬────────────┐
    │ Model       │ HIGH PASS  │ LOW PASS   │  FAIL    │ Pass Rate  │
    ├─────────────┼────────────┼────────────┼──────────┼────────────┤
"""
...
for model in models:
    m_df = df[df['Model'] == model]
    total = len(m_df)
    high_pass = (m_df['Final_Quality_Rating'] == 'HIGH PASS').sum()
    low_pass = (m_df['Final_Quality_Rating'] == 'LOW PASS').sum()
    fail = (m_df['Final_Quality_Rating'] == 'FAIL').sum()
    pass_rate = (high_pass + low_pass) / total * 100
    report += f"    │ {MODEL_NAMES[model]:<11} │ {high_pass:>10} │ {low_pass:>10} │ {fail:>8} │ {pass_rate:>9.1f}% │\n"
...
report += """    └─────────────┴────────────┴────────────┴──────────┴────────────┘
    ...
    ┌─────────────┬────────────────────────┬────────────┐
    │ Model       │ Chunks with Critical   │ Percentage │
    ├─────────────┼────────────────────────┼────────────┤
"""
...
for model in models:
    m_df = df[df['Model'] == model]
    total = len(m_df)
    critical = (m_df['Critical_Error'] == True).sum()
    pct = critical / total * 100
    report += f"    │ {MODEL_NAMES[model]:<11} │ {critical:>22} │ {pct:>9.1f}% │\n"

report += """    └─────────────┴────────────────────────┴────────────┘

    Note: A translation FAILS if it has Critical Errors, regardless of TQS score.
    LOW PASS requires TQS >= 85 with no critical errors.
    HIGH PASS requires TQS >= 93 with no critical errors.
"""
...
═══════════════════════════════════════════════════════════════════════════════
    ...
    5. chart5_quality_ratings.png - Quality rating distribution & pass rates
